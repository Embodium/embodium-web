...
export const WebDocument = component$<DocumentDescriptor>((props: DocumentDescriptor) => {
  ...
  return <>
    <head>
      ...
      {props?.metadata?.pragma?.refresh && typeof props?.metadata?.pragma?.refresh === "number" && <meta http-equiv="refresh" content={`${(props?.metadata?.pragma?.refresh ?? -1)}`} />}
      {props?.metadata?.pragma?.refresh && typeof props?.metadata?.pragma?.refresh === "object" && <meta http-equiv="refresh" content={`${props?.metadata?.pragma?.refresh?.interval};${props?.metadata?.pragma?.refresh?.url}`} />}
